...
class VeilThemeColors {
  ...
  // Screen-name strip (below title bar)
  final Color nameBg;

  // Section header ("Conversations (n)")
  final Color sectionBg;
  final Color sectionText;

  // Conversation rows
  final Color listBg;       // background behind the rows
  final Color rowBg;        // individual row background
  final bool roundedRows;
  final double rowRadius;

  // Avatar circle
  final bool gradientAvatar; // true = per-letter gradient; false = solid color
  final Color solidAvatarBg; // used when gradientAvatar = false
  ...
  // ── Per-letter gradient palette (Dark + Glass themes) ─────────────────
  ...
  static const VeilThemeColors glass = VeilThemeColors(
    ...
    titleStart:     Color(0x14FFFFFF), // white @ 8%
    titleEnd:       Color(0x0AFFFFFF), // white @ 4%
    nameBg:         Color(0x0FFFFFFF),
    sectionBg:      Colors.transparent,
    sectionText:    Color(0xFF9CA3AF),
    listBg:         Colors.transparent,
    rowBg:          Color(0x0FFFFFFF), // white @ 6%
    roundedRows:    true,
    rowRadius:      16,
    gradientAvatar: true,
  ...
  static const VeilThemeColors light = VeilThemeColors(
    ...
    titleStart:     Color(0xFFDFEEFD),
    titleEnd:       Color(0xFFFFFFFF),
...
class VeilThemeNotifier extends ChangeNotifier {
  VeilThemeMode _mode = VeilThemeMode.aim;
  VeilThemeMode get mode => _mode;
  VeilThemeColors get colors => VeilThemeColors.forMode(_mode);

  VeilThemeNotifier() { _load(); }

  Future<void> _load() async {
    final p = await SharedPreferences.getInstance();
    final saved = p.getString('veil_app_theme') ?? 'aim';
    _mode = VeilThemeMode.values.firstWhere(
      (e) => e.name == saved,
  ...
  Future<void> setMode(VeilThemeMode mode) async {
    if (_mode == mode) return;
    _mode = mode;
    notifyListeners();
    final p = await SharedPreferences.getInstance();
    await p.setString('veil_app_theme', mode.name);
  }

  void cycle() {
    final next = VeilThemeMode.values[(_mode.index + 1) % VeilThemeMode.values.length];
    setMode(next);
  }
}
